...
//New UI Stuff - Ammar NVM

// import editors.ui.TopBar;
...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nUsed on Fresh during the beatbox parts.fnfnWarning: Value must be integer!"],
	...
	var dummyArrowImage:FlxSprite;
	var dummyArrowArray:Array<String> = ['right', 'down', 'up', 'left'];
	...
	**/
	...
	//AMMAR ADDONS
	private var noteColor:Array<FlxColor> = [
		0xFFC24B99,
		0xFF1FE3F1,
		0xFF0EF106,
		0xFFF9393F
	...
	private var curPrevBeat:Int = 0;
	var mouseGraphics:Map<String, BitmapData> = new Map<String, BitmapData>(); //idle, select, text
	var mouseSprite:FlxSprite; //idle, select, text
	var sprSelectables:Array<FlxSprite> = [];

	var vocalKeybinds:Array<Dynamic> = [];

	var instVol:Float = 1;
	var allVoicesVol:Float = 1;
	...
	override function create()
	{
		...
		vocalKeybinds = [
			...
			ClientPrefs.copyKey(ClientPrefs.keyBinds.get('vocal_ee')),
			ClientPrefs.copyKey(ClientPrefs.keyBinds.get('vocal_oo')),
			ClientPrefs.copyKey(ClientPrefs.keyBinds.get('vocal_eh'))
		];
		...
		if (_song.vocals == null) {
			_song.vocals = [];
			trace('no way, adding vocals array,,,');
		}

		difficulty = CoolUtil.difficulties[PlayState.storyDifficulty];
		...
		cameraIcon = new FlxSprite(0, 0).loadGraphic(Paths.image('camera'));
		leftIcon = new HealthIcon('bf');
		rightIcon = new HealthIcon('dad');
		rightIcon.flipX = true;
		cameraIcon.scrollFactor.set(0, 0);
		eventIcon.scrollFactor.set(0, 0);
		leftIcon.scrollFactor.set(0, 0);
		rightIcon.scrollFactor.set(0, 0);

		cameraIcon.setGraphicSize(60, 60);
		...
		//leftIcon.setPosition(gridBG.x/*GRID_SIZE + 10*/, -100);
		...
		var so:FlxSprite = new FlxSprite().loadGraphic(Paths.image('cursor/idle'));
		mouseGraphics['idle'] = so.pixels;

		so.loadGraphic(Paths.image('cursor/select'));
		mouseGraphics['select'] = so.pixels;

		so.loadGraphic(Paths.image('cursor/text'));
		mouseGraphics['text'] = so.pixels;

		so.loadGraphic(Paths.image('cursor/add'));
		mouseGraphics['add'] = so.pixels;

		loadCursor('idle');

		//FlxG.mouse.load(mouseSprite.framePixels);
		...
		var idk:Float = gridBG.width/2;
		leftIcon.setPosition(gridBG.x + idk - 45 - 25 /*GRID_SIZE + 10*/, 50);
		rightIcon.setPosition(gridBG.x + idk + gridBG.width + 45/*GRID_SIZE * 5.2*/, 50);
		cameraIcon.y = 150;
		updateCameraIcon(1);

		bpmTxt = new FlxText(20, 550, 0, "", 16);
		bpmTxt.scrollFactor.set();
		add(bpmTxt);

		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE * 10), 4);
		add(strumLine);
		...
		dummyArrowImage = new FlxSprite();
		dummyArrowImage.frames = Paths.getSparrowAtlas("NOTE_assets");
		dummyArrowImage.animation.addByPrefix('left', 'red0', 0, true);
		dummyArrowImage.animation.addByPrefix('down', 'blue0', 0, true);
		dummyArrowImage.animation.addByPrefix('up', 'green0', 0, true);
		dummyArrowImage.animation.addByPrefix('right', 'purple0', 0, true);
		dummyArrowImage.animation.play('left', true);
		add(dummyArrowImage);

		dummyArrowImage.antialiasing = false;
		dummyArrowImage.setGraphicSize(GRID_SIZE, GRID_SIZE);
		dummyArrowImage.alpha = 0.4;
		dummyArrowImage.visible = false;
		dummyArrowImage.offset.set(58, 58);
		...
		difficultySong = new FlxText(10, 10 + 16 + 8, 0, "Difficulty: " + difficulty, 16);
		difficultySong.scrollFactor.set();
		add(difficultySong);

		// addTopBar();
		// topUI.findComponent("playervol").disabled = !separateVocals;
		// topUI.findComponent("opponentvol").disabled = !separateVocals;
		// topUI.findComponent("voicesvol").disabled = separateVocals;
	...
	// var topUI:TopBar;
	// function addTopBar():Void {
	// 	topUI = new TopBar();
    // 	add(topUI);

	// 	topUI.findComponent("save").onClick = (_) -> {
	// 		saveLevelInstant();
	// 		NotificationManager.instance.addNotification({
	// 			title: "Your Data is save!",
	// 			body: "Your data have been save in "+_song.song
	// 		});
	// 	}
	// 	topUI.findComponent("saveas").onClick = (_) -> {saveLevel();}
	// 	topUI.findComponent("loadaudio").onClick = (_) -> {
	// 		currentSongName = Paths.formatToSongPath(UI_songTitle.text);
	// 		loadSong();
	// 		updateWaveform();
	// 		NotificationManager.instance.addNotification({
	// 			title: "Audio has been reloaded",
	// 			body: "reloaded"
	// 		});
	// 	}
	// 	topUI.findComponent("loaddata").onClick = (_) -> {
	// 		openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){
	// 			loadJson(_song.song.toLowerCase()); 
	// 			NotificationManager.instance.addNotification({
	// 				title: "Data has been reloaded",
	// 				body: "reloaded"
	// 			});
	// 		}, null,ignoreWarnings));
	// 	}
	// 	topUI.findComponent("loadevent").onClick = (_) -> {
	// 		reloadEvents();
	// 		NotificationManager.instance.addNotification({
	// 			title: "Events has been reloaded",
	// 			body: "reloaded"
	// 		});
	// 	}

	// 	topUI.findComponent("instvol").onChange = (_) -> { // 0 - 100
	// 		instVol = topUI.findComponent('instvol').value/100;
	// 		FlxG.sound.music.volume = instVol;
	// 	}

	// 	topUI.findComponent("voicesvol").onChange = (_) -> { // 0 - 100
	// 		allVoicesVol = topUI.findComponent('voicesvol').value/100;
	// 		updateVolume();
	// 	}

	// 	topUI.findComponent("opponentvol").onChange = (_) -> { // 0 - 100
	// 		oppVoicesVol = topUI.findComponent('opponentvol').value/100;
	// 		updateVolume();
	// 	}

	// 	topUI.findComponent("playervol").onChange = (_) -> { // 0 - 100
	// 		voicesVol = topUI.findComponent('playervol').value/100;
	// 		updateVolume();
	// 	}


	// 	topUI.findComponent("name").onChange = (_) -> {
	// 		trace(topUI.findComponent("name").text);
	// 	}
	...
	function addSectionUI():Void
	{
		...
		var check_vocalsSec:FlxUICheckBox = null;
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Clear", function()
		{
			...
			if(check_vocalsSec.checked)
				{
					var i:Int = _song.vocals.length - 1;
					var startThing:Float = sectionStartTime();
					var endThing:Float = sectionStartTime(1);
					while(i > -1) {
						var event:Array<Dynamic> = _song.vocals[i];
						if(event != null && endThing > event[0] && event[0] >= startThing)
						{
							_song.vocals.remove(event);
						}
						--i;
					}
				}
		...
		check_vocalsSec = new FlxUICheckBox(check_eventsSec.x + 100, check_notesSec.y, null, null, "Vocals", 75);
		check_vocalsSec.checked = true;
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "Copy last section", function()
		{
			...
			if (check_notesSec.checked) 
				for (note in _song.notes[daSec - value].sectionNotes)
				{
					...
					//if ((note[1] < 0 && check_eventsSec.checked) || (note[1] > 7 && check_vocalsSec.checked) || (check_notesSec.checked)) {
			...
			if (check_eventsSec.checked) 
				...
				{
					var strumTime:Float = event[0];
					if((endThing+1) > event[0] && event[0] >= startThing)
					{
			...
			if (check_vocalsSec.checked) 
				for (event in _song.vocals)
					{
						var strumTime:Float = event[0];
						if((endThing+1) > event[0] && event[0] >= startThing)
						{
							strumTime += Conductor.stepCrochet * (getSectionBeats(daSec) * 4 * value);
							_song.vocals.push([strumTime, event[1], event[2]]);
						}
					}
			updateGrid(true);
		});
		...
		tab_group_section.add(check_vocalsSec);
	...
	var check_deleteOnlyNoteType:FlxUICheckBox;
	var check_showOnlyNoteType:FlxUICheckBox;
	var check_ignoreAddNotes:FlxUICheckBox;
	...
	function addNoteUI():Void
	{
		...
		noteTypeDropDown = new FlxUIDropDownMenuCustom(10, 105, FlxUIDropDownMenuCustom.makeStrIdLabelArray(displayNameList, true), function(character:String)
		{
			...
			currentNoteType = noteTypeIntMap.get(currentType);
			trace(currentNoteType);
		});
		blockPressWhileScrolling.push(noteTypeDropDown);

		check_deleteOnlyNoteType = new FlxUICheckBox(140, noteTypeDropDown.y, null, null, "Delete Only Note Type", 100);
		check_deleteOnlyNoteType.name = 'check_deleteNoteType';
		check_deleteOnlyNoteType.checked = false;

		check_showOnlyNoteType = new FlxUICheckBox(140, check_deleteOnlyNoteType.y + 25, null, null, "Show Only Note Type", 100);
		check_showOnlyNoteType.name = 'check_showNoteType';
		check_showOnlyNoteType.checked = false;

		check_ignoreAddNotes = new FlxUICheckBox(140, check_showOnlyNoteType.y + 25, null, null, "Ignore Notes when adding Notes", 100);
		check_ignoreAddNotes.name = 'check_ignoreAddNotes';
		check_ignoreAddNotes.checked = false;
		...
		tab_group_note.add(strumTimeInputText);
		tab_group_note.add(check_deleteOnlyNoteType);
		tab_group_note.add(check_showOnlyNoteType);
		tab_group_note.add(check_ignoreAddNotes);
	...
	function addChartingUI() {
		...
		playSoundDad = new FlxUICheckBox(playSoundBf.x + 120, playSoundBf.y, null, null, 'Play Sound (Opponent notes)', 100,
			function() {
		...
		opponentVolume = new FlxUINumericStepper(voicesVolume.x + 100, voicesVolume.y, 0.1, 1, 0, 1, 1);
		opponentVolume.value = vocals.volume;
		opponentVolume.name = 'oppVoices_volume';
		...
		tab_group_chart.add(new FlxText(opponentVolume.x, opponentVolume.y - 15, 0, 'Opp Volume'));
	...
	function loadSong():Void
	{
		...
		var hardVocals:Bool = (difficulty.toLowerCase() == 'hard');

		separateVocals = false;


		if (hardVocals && Paths.voicesExist(currentSongName, '-hard')) {
			trace('yes hard voices');
			vocals.loadEmbedded(Paths.voices(currentSongName, '-hard'));
			FlxG.sound.list.add(vocals);

			hardVocals = false; //no need to search again for separate
		}
		else if(Paths.voicesExist(currentSongName)) {
			vocals.loadEmbedded(file);
			FlxG.sound.list.add(vocals);
		}

		if (Paths.voicesExist(currentSongName, '-player')) {
			if (Paths.voicesExist(currentSongName, '-opponent')) {
				vocals.loadEmbedded(Paths.voicesPlayer(currentSongName));
				opponentVocals.loadEmbedded(Paths.voicesOpponent(currentSongName));
				separateVocals = true;
				FlxG.sound.list.add(vocals);
		...
		if (!separateVocals) {
			if (hardVocals) {
				if (!Paths.voicesExist(currentSongName, '-opponent-hard') || !Paths.voicesExist(currentSongName, '-player-hard')) {
					trace('no hard separate voices song found ');
				} else {
					trace('saperate Hard Mode Voices!!');
					separateVocals = true;
					opponentVocals.loadEmbedded(Paths.voicesOpponent(currentSongName, '-hard'));
					vocals.loadEmbedded(Paths.voicesPlayer(currentSongName, '-hard'));
				}
			} 
		}
	...
	function generateSong() {
		var hardInst:Bool = (difficulty.toLowerCase() == 'hard');
		if (hardInst && !Paths.instExist(currentSongName, '-hard')) {
			hardInst = false;
			trace('no hard inst');
        } 
		FlxG.sound.playMusic(Paths.inst(currentSongName, hardInst ? '-hard' : ''), 0.6/*, false*/);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'oppVoices_volume')
			{
				if (separateVocals) 
	...
	override function update(elapsed:Float)
	{
		...
		//trace(topUI.findComponent("name").inFocus);
			...
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom])
		{
			...
			var data:Int = Math.floor(FlxG.mouse.x / GRID_SIZE); // 0 = events, 1 = note0
			...
			if (data > 0 && data < 9) {
				dummyArrowImage.visible = true;
				dummyArrowImage.setPosition(dummyArrow.x, dummyArrow.y);
				dummyArrowImage.animation.play(dummyArrowArray[(data-1)%4]);
				dummyArrow.alpha = 0;
			} else {
				dummyArrowImage.visible = false;
				dummyArrow.alpha = 1;
		...
		{
			if (FlxG.mouse.overlaps(curRenderedNotes) && !check_ignoreAddNotes.checked)
			{
				var breakFunction:Bool = false;
				curRenderedNotes.forEachAlive(function(note:Note)
				{
					if (FlxG.mouse.overlaps(note) && !breakFunction)
					{
						if (!FlxG.mouse.justPressedRight && FlxG.keys.pressed.CONTROL)
						{
							selectNote(note);
						}
						else if (!FlxG.mouse.justPressedRight && FlxG.keys.pressed.ALT)
						{
							selectNote(note);
							curSelectedNote[3] = noteTypeIntMap.get(currentType);
							updateGrid(true);
		...
		if (!blockInput)
		{
			...
			if (FlxG.mouse.x > gridBG.x
				...
				&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom])
			{
				for (i in 0...vocalKeybinds.length)
				{
					if (FlxG.keys.anyJustPressed(vocalKeybinds[i])) {
						addNote(null, null, null, i);
					}
		...
		strumLine.visible = !strumLineNotes.visible;
		...
		if (curPrevBeat != curBeat) {
			curPrevBeat = curBeat;
			leftIcon.scale.set(0.8, 0.8);
			rightIcon.scale.set(0.8, 0.8);
		} else {
			var scale:Float = FlxMath.lerp(leftIcon.scale.x, 0.6, FlxMath.bound(elapsed * 10, 0 , 1));
			leftIcon.scale.set(scale, scale);
			rightIcon.scale.set(scale, scale);
		}
		updateCameraIcon(elapsed*13);
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if (check_showOnlyNoteType.checked && note.noteType != currentNoteType)
				note.alpha = 0.2;
			if(curSelectedNote != null) {
			...
			if(note.strumTime <= Conductor.songPosition) {
				//note.alpha = 0.4;
				if (!check_showOnlyNoteType.checked)
					note.alpha = 0.5;
				else {
					if (note.noteType == currentNoteType)
						note.alpha = 0.5;
					else 
						note.alpha = 0.2;
				}
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && noteDataToCheck < 8 && note.mustPress) noteDataToCheck += 4;
					if (noteDataToCheck < 8) {
						...
						if(!playedSound[data]) {
							if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
								...
								FlxG.sound.play(Paths.sound(soundToPlay), 0.5).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
	...
	*/
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 11, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 11, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
			
			prevGridBG =  FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 11, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
			leHeight = Std.int(gridBG.height + nextGridBG.height + prevGridBG.height);
			foundNextSec = true;
		}
		else {
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE * 11), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 6), -prevGridBG.height).makeGraphic(2, leHeight, FlxColor.BLACK);
		gridLayer.add(gridBlackLine); //MID LINE
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE, -prevGridBG.height).makeGraphic(2, leHeight, FlxColor.BLACK);
		gridLayer.add(gridBlackLine); //EVENTS

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE*2), -prevGridBG.height).makeGraphic(2, leHeight, FlxColor.BLACK);
		gridLayer.add(gridBlackLine); //RIGHT LINE
		updateGrid();
	...
	function updateGrid(onlyMainSec:Bool = false):Void
	{
		//curRenderedNotes.kill();
		//nextRenderedNotes.kill();
		//curRenderedNotes.forEachAlive(function(mem:Note){mem.kill();});
		//nextRenderedNotes.forEachAlive(function(mem:Note){mem.kill();});
		...
		{
			var note:Note = setupNoteData(i, false, false);
		...
		var endThing:Float = sectionStartTime(1)-1;
		...
		//CURRENT VOCALS
		if (_song.vocals != null && _song.vocals.length > 0)
			for (i in _song.vocals)
				{
					if(endThing > i[0] && i[0] >= startThing)
					{
						var note:Note = setupNoteData(i, false, false, true);
						curRenderedNotes.add(note);
						//trace('test: ' + i[0], 'startThing: ' + startThing, 'endThing: ' + endThing);
					}
				}
		...
		if(curSec < _song.notes.length-1 && !onlyMainSec) {
			if (_song.notes[curSec+1] != null)
		...
		// PREV SECTION
		var beats:Float = getSectionBeats(1);
		if(curSec < _song.notes.length-2 && !onlyMainSec) {
			if (_song.notes[curSec-1] != null) 
				for (i in _song.notes[curSec-1].sectionNotes)
				{
		...
		// NEXT EVENTS
		var startThing:Float = sectionStartTime(1)-1;
		var endThing:Float = sectionStartTime(2)-1;
		if (!onlyMainSec)
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
		...
		//NEXT VOCALS
		if (_song.vocals != null && _song.vocals.length > 0 && !onlyMainSec)
			for (i in _song.vocals)
				{
					if(endThing > i[0] && i[0] >= startThing)
					{
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool, isPrevSection:Bool = false, vocalNote:Bool = false):Note
	{
		var daNoteInfo = i[1];
		var daStrumTime = i[0];
		var daSus:Dynamic = i[2];

		var note:Note = /*group.recycle(Note);*/ new Note(daStrumTime, daNoteInfo % 4, null, null, true);
		//note.setupNote(daStrumTime, daNoteInfo % 4, null, null, true);
		if(daSus != null && !vocalNote) { //Common note
			...
			note.noteType = i[3];
			if (note.noteType != currentNoteType && check_showOnlyNoteType != null)
				if (check_showOnlyNoteType.checked)
				note.alpha = 0.2;
				
		} else if (vocalNote) { //Event note
			note.loadGraphic(Paths.image('vocals'), true, 128, 128);
			note.animation.add('idle', [0, 1, 2, 3], 0, true);
			note.animation.play('idle', true);
			note.animation.frameIndex = daNoteInfo;
			note.vocalData = daNoteInfo; 
			note.vocalEnemy = i[2];
			note.isVocal = true;
			note.noteData = 8 + (i[2] ? 0 : 1);
			daNoteInfo = 8 + (i[2] ? 0 : 1);
		}else { //Event note
		...
		if (!vocalNote) {
			if (_song.notes[curSec].mustHitSection && !isNextSection && !isPrevSection) {
				if(daNoteInfo > 3) { 
					note.x -= GRID_SIZE * 4;
				} else if(daSus != null) {
					note.x += GRID_SIZE * 4;
			...
			if (_song.notes[curSec+1] != null)
				if (_song.notes[curSec+1].mustHitSection && isNextSection && !isPrevSection) {
					if(daNoteInfo > 3) { 
						note.x -= GRID_SIZE * 4;
					} else if(daSus != null) {
						note.x += GRID_SIZE * 4;
			...
			if (_song.notes[curSec-1] != null)
				if ( _song.notes[curSec-1] != null && _song.notes[curSec-1].mustHitSection && !isNextSection && isPrevSection) {
					if(daNoteInfo > 3) { 
						note.x -= GRID_SIZE * 4;
					} else if(daSus != null) {
						note.x += GRID_SIZE * 4;
		...
		if(note.y < -150 && !isPrevSection && !isNextSection) note.y = -150;
		return note;
	}
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite {
		...
		spr.color = noteColor[note.noteData % 4];
		spr.alpha = 0.5;
		return spr;
	}

	function updateCameraIcon(elapsed:Float = 0):Void {
		var target:FlxSprite = (_song.notes[curSec].mustHitSection ? rightIcon : leftIcon);
		
		cameraIcon.x = target.x + (target.width/2) - (cameraIcon.width/8);
	}
	...
	function selectNote(note:Note):Void
	{
		...
		if(noteDataToCheck > -1 && ((!check_showOnlyNoteType.checked) || (check_showOnlyNoteType.checked && note.noteType == currentNoteType)))
		{
	...
	{
		var noteDataToCheck:Int = note.noteData;
		var rightButton:Bool = FlxG.mouse.justPressedRight;
		if(noteDataToCheck > -1 && noteDataToCheck < 8 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 4;

		if(!rightButton && note.noteData > -1 && note.noteData < 8 && ((!check_deleteOnlyNoteType.checked) || (check_deleteOnlyNoteType.checked && note.noteType == currentNoteType))) //Normal Notes
		{
			for (i in _song.notes[curSec].sectionNotes)
			{
		...
		else if (!rightButton && note.noteData == -1) //Events
		{
			for (i in _song.events)
			{
		...
		else if (note.isVocal) //Sing Note
		{
			for (i in _song.vocals)
				{
					if(i[0] == note.strumTime && i[2] == (note.vocalEnemy))
					{
						if (i[1] < 3 && !rightButton) i[1] += 1;
						else
							_song.vocals.remove(i);
						break;
					}
				}
		}
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null, ?vocalType:Int = 0):Void
	{
		//curUndoIndex++;
		...
		if(noteData > -1 && noteData < 8)
		{
			noteData = (noteData + (_song.notes[curSec].mustHitSection ? 4 : 0)) % 8;
		...
		{
			trace('sing Note');
			_song.vocals.push([noteStrum, vocalType, noteData == 8]);
			updateGrid(true);
		}

		if (FlxG.keys.pressed.CONTROL && noteData > -1 && noteData < 8)
		{
	...
	function loadJson(song:String):Void
	{
		...
		if (CoolUtil.difficulties[PlayState.storyDifficulty] != CoolUtil.defaultDifficulty) {
			if(CoolUtil.difficulties[PlayState.storyDifficulty] == null){
				...
				trace('Difficulty is null');
			}else{
				PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + CoolUtil.difficulties[PlayState.storyDifficulty], song.toLowerCase());
				trace(CoolUtil.difficulties[PlayState.storyDifficulty]);
				difficulty = CoolUtil.difficulties[PlayState.storyDifficulty];
			}
		}else{
		trace('sad no difficulty');
		PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
		}
		MusicBeatState.resetState();
	}
	...
	private function saveLevelInstant()
		{
			if(_song.events != null && _song.events.length > 1) _song.events.sort(sortByTime);
			var json = {
				"song": _song
			...
			if ((data != null) && (data.length > 0))
			{
				...
				//_file.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
				//_file.save(data.trim(), Paths.formatToSongPath(_song.song) + ".json");
				var songPath:String = Paths.formatToSongPath(_song.song);
				var location:String = Paths.getPreloadPath("data/"+songPath+"/"+songPath + "-test.json");
				trace(location);
				#if sys
				sys.io.File.saveContent(location, data.trim());
				#end
			}
		}
	...
	function loadCursor(name:String = 'idle'):Void {
		FlxG.mouse.load(mouseGraphics[name], 0.5, Std.int(-75*0.5), Std.int(-75*0.5));
	}

	function updateVolume():Void {
		vocals.volume = (separateVocals ? voicesVol : allVoicesVol);
		if (separateVocals) 
			opponentVocals.volume = (separateVocals ? oppVoicesVol : allVoicesVol);
	}
}
