...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (PlayState.SONG.needsVoices) {
			if(Paths.voicesExist(PlayState.SONG.song, '')) {
				trace('found');
				vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song)); }
			else
				vocals = new FlxSound();
			
			if(Paths.voicesExist(PlayState.SONG.song, '-opponent')) {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image('numCombo/' + pixelShitPart1 + 'num' + Std.int(i) + pixelShitPart2));
