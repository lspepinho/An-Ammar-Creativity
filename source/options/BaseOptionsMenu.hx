...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	var holdPress:Array<Bool> = [false, false];
	...
	public function new()
	{
		...
		leftButton = new FlxSprite().loadGraphic(Paths.image('mobile/left'));
        leftButton.setGraphicSize(Std.int(leftButton.width * 0.5)); leftButton.updateHitbox();
        leftButton.x = 1280-(leftButton.width*2); leftButton.y = 720 - leftButton.height;
        leftButton.alpha = 0.4;
		
		rightButton = new FlxSprite().loadGraphic(Paths.image('mobile/right'));
        rightButton.setGraphicSize(Std.int(rightButton.width * 0.5)); rightButton.updateHitbox();
        rightButton.x = 1280-rightButton.width; rightButton.y = 720 - leftButton.height;
		...
		//DOWN , UP
		FlxMouseEvent.add(leftButton, function(_){ //down
			holdPress[0] = true;
		}, function(_){ //up
			holdPress[0] = false;
			clearHold();
		...
		FlxMouseEvent.add(rightButton, function(_){ //down
			holdPress[1] = true;
		}, function(_){ //up
			holdPress[1] = false;
			clearHold();
	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
			...
			if(usesCheckbox)
			{
				...
				var left:Bool = controls.UI_LEFT || holdPress[0];
				var right:Bool = controls.UI_RIGHT || holdPress[1];
				var leftP:Bool = controls.UI_LEFT_P || holdPress[0];
				var rightP:Bool = controls.UI_RIGHT_P || holdPress[1];
				if(left || right) {
