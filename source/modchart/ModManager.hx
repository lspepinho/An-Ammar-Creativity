...
class ModManager {
	public var modchartEnable:Bool = false;
    ...
    inline public function setPercent(modName:String, val:Float, player:Int=-1)
		setValue(modName, val*0.01, player);
	...
	public function ease(beat:Float, beatDur:Float, modName:String, target:Float, style:String = 'linear', player:Int = -1, ?startVal:Float) // BEAT VERSION!!! because me like --ammar
		queueEase(beat*4, ((beat*4) + (beatDur*4)), modName, target, style, player);
	
	public function set(beat:Float, modName:String, target:Float, player:Int = -1)
		queueSet(beat*4, modName, target, player);
