...
class FunkinLua {
	...
	public function new(script:String) {
		...
		// MOD Saves'
		set('DifficultyOption', (ClientPrefs.aDifficulty).toLowerCase());
		...
		set('SpecialNotes', ClientPrefs.getGameplaySetting('specialnotes', true));
		set('Modchart', ClientPrefs.getGameplaySetting('modchart', true));
		// Lua shit
		...
		Lua_helper.add_callback(lua, "multiThread", function(name:String)
		{
			#if (target.threaded)
			sys.thread.Thread.create(() -> {
				PlayState.instance.callOnLuas('multiThread', [name]);
			});
			return true;
			#else
			PlayState.instance.callOnLuas('multiThread', [name]);
			luaTrace("multiThread: Error multi thread is not available.", false, false, FlxColor.RED);
			return false;
			#end
		});
		...
		Lua_helper.add_callback(lua, "getColorFromHex", function(color:String) {
			...
			var result:Int = Std.parseInt(color);
			return result;
		});
		...
		Lua_helper.add_callback(lua, "cameraFlash", function(camera:String, color:String, duration:Float,forced:Bool) {
			var colorNum:Float = Std.parseInt(color);
			if(!color.startsWith('0x')) colorNum = Std.parseInt('0xff' + color);
			cameraFromString(camera).flash(Std.int(colorNum), duration,null,forced);
		});
		Lua_helper.add_callback(lua, "cameraFade", function(camera:String, color:String, duration:Float,forced:Bool) {
			var colorNum:Float = Std.parseInt(color);
			if(!color.startsWith('0x')) colorNum = Std.parseInt('0xff' + color);
			cameraFromString(camera).fade(Std.int(colorNum), duration,false,null,forced);
		});
		...
		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int, height:Int, color:String) {
			...
			if(spr!=null) {
				PlayState.instance.getLuaObject(obj,false).makeGraphic(width, height, Std.int(colorNum));
				return;
			}

			var object:FlxSprite = Reflect.getProperty(getInstance(), obj);
			if(object != null) {
				object.makeGraphic(width, height, Std.int(colorNum));
			}
		});
			...
			if(PlayState.instance.getLuaObject(obj, false) != null) {
				...
				if(luaObj.animation.getByName(name) != null)
				{
					...
					if(Std.isOfType(luaObj, ModchartSprite))
					{
						...
						{
							luaObj.offset.set(daOffset[0] * luaObj.scale.x, daOffset[1] * luaObj.scale.y);
						}
					}
				}
				return true;
		...
		Lua_helper.add_callback(lua, "setHealthBarColors", function(leftHex:String, rightHex:String) {
			var left:FlxColor = Std.int(Std.parseInt(leftHex));
			if(!leftHex.startsWith('0x')) left = Std.int(Std.parseInt('0xff' + leftHex));
			var right:FlxColor = Std.int(Std.parseInt(rightHex));
			if(!rightHex.startsWith('0x')) right = Std.int(Std.parseInt('0xff' + rightHex));
		...
		Lua_helper.add_callback(lua, "setTextBorder", function(tag:String, size:Int, color:String) {
			...
			if(obj != null)
			{
				...
				obj.borderSize = size;
				obj.borderColor = Std.int(colorNum);
				return true;
			}
