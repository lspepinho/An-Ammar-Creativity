...
uniform float FLIGHT_SPEED;// = 16.0;

uniform float DRAW_DISTANCE;// = 60.0; // Lower this to increase framerate
uniform float FADEOUT_DISTANCE;// = 10.0; // must be < DRAW_DISTANCE    
uniform float FIELD_OF_VIEW;// = 1.05;   

uniform float STAR_SIZE;// = 0.6; // must be > 0 and < 1
uniform float STAR_CORE_SIZE;// = 0.14;

uniform float CLUSTER_SCALE;// = 0.02;
uniform float STAR_THRESHOLD;// = 0.775;

uniform float BLACK_HOLE_CORE_RADIUS;// = 0.2;
uniform float BLACK_HOLE_THRESHOLD;// = 0.9995;
uniform float BLACK_HOLE_DISTORTION;// = 0.03;
